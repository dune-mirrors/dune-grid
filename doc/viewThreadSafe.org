...
** Explicit and Implicit Call Arguments
   ...
   which prohibits any concurrent calls that operate on the same argument, or
   shared, which allows other concurrent shared operations.  Sometimes and
...
** Access Modes
   ...
   - none :: The call does not access this argument in any way.  Concurrent
             calls using this argument may happen in any mode.
   - shared :: The call reads the argument, but does not modify it, or
               otherwise ensures that multiple concurrent shared calls are
               safe.
   - exclusive :: The call modifies the argument.  No concurrent exclusive or
                  shared accesses are permissible, but concurrent calls with
...
** Caches
   ...
   of the other cache below, since this may race with concurrent "may-init" or
...
** Communication
   ...
   communications concurrently.  The rule therefore is: there may be at most
   ...
   do many other things concurrently to to communication, and thus attempt
   ...
   communication is happening concurrently.  To express this, such calls are
...
** GridView
   ...
   sure that no other cache of the grid is potentially initialized
   concurrently e.g. via other grid views, access to the id sets, or
   communication.
...
** Intersections and Intersection Iterators
   ...
   flipped, A and B may be access concurrently and their caches are seperate.
...
** Prefer Operations on the Grid Views over Opperations on the Grid
   ...
   concurrently obtained iterators, grid views, index sets, and id sets as
   well as concurrent communication is complex.
** Avoid Re-Obtaining the Same Grid View.
   ...
   can.
** You may do one communication concurrently with many other threadsafe operations
...
** Explicitly Initialize any potentially used Index/ID Sets in Serial Section
   ...
   Even different index/id sets should not be initialized concurrently, since
   ...
   concurrently to initialization.
** You are free to obtain entity iterators concurrently
   You may obtain iterators from the same grid view in different threads
   concurrently.  Alternatively, each thread may have it's own copy of the
   grid view, and may obtain iterators from it.
** Operations on objects obtained from iterators/pointers are exclusive
   Specifically, this means: when you have dereference an entity
   iterator/pointer, keep all subsequent operations on the entity and objects
   derived from the entity in the same thread, until you are done with the
   entity for the time being.  "Objects derived from the entity" means the
   entity itself, any intersection iterators and intersections, any
   geometries, any hierarchic iterators and entities obtained from the
   hierarchic iterators via dereferencing.

   The exception to this rule are entity pointers obtained through father,
   inside, and outside, as well as entity pointers constructed with an entity
   as argument.  Such entity pointers should still be constructed in the same
   thread, but with sufficient synchronization they may be passed to a
   different thread.
** Entity iterators and pointers may be concurrently copied
   This is true as long as only copy operations with the same source
   entity object/entity pointer object/entity iterator object happen
   concurrently.  Other operations on the source, such as obtaining the
   geometry or intersection iterators should not be attempted concurrently.
* 
